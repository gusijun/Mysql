...
分类：六大约束
		not null:非空,用户保证该字段的值不能为空
		比如姓名、学号等
		default:默认,用户保证该字段有默认值
		比如性别
		primary key:主键,保证该字段的值具有唯一性,并且非空
		比如学号、员工编号等
		unique：唯一,用户保证该字段的值具有唯一性,可以为空
		比如座位号
		check:检查约束【mysql不支持】
		比如年龄、性别
		foreign key：外键,用户限制两个表的关系,用户保证该字段的值必须来于主表
					的关联列的值,在从表添加外键约束,用于引用主表某列的值
...
外键：
	...
	delete from stuinfo;
	insert into stuinfo values(1,'John','男',null,18,1,1);
...
CREATE TABLE stuinfo(
	...
	marjorId INT  REFERENCES major(id) #外键
);

...
FROM employees;
二、查询员工表的employee_id,job_id,last_name
按department_id降序,salary升序
...
五、显示当前日期,以及去除前后空格,截取子字符串的函数
SELECT NOW();
SELECT TRIM(字符 FROM '');
SELECT SUBSTR(str,strartIndex);
SELECT SUBSTR(str,statrtIndex,`length`);
